...
func TestNewGaugeServer(t *testing.T) {
	_, err := NewGaugerServer(context.Background(), ":8080", ":3333", "tmp.file", false, 1*time.Minute, "", "key", "", "")

	require.NoError(t, err)
}

func TestRouter(t *testing.T) {
	server, err := NewGaugerServer(context.Background(), "localhost:8088", ":3333", "temp.txt", true, 1*time.Minute, "", "cxvxv", "", "")
	require.NoError(t, err)
